...
for i in range(4):
    ...
    t_file, loss_gcn_file, perf_min_file, perf_gcn_file, perf_ran_file = np.loadtxt(
        './results/logs/val_epochs_one_step/log_supervise_val_per_epochs20_nondet_0001_one_step_greedy_without_prune_' + str(
            val_set[i]) + '_logsoftmax_fulldepth.txt', delimiter=' ', usecols=(1, 3, 5, 7, 9), unpack=True)

    row_max = t_file.size
    t[i]= np.arange(21)
    loss_gcn[i][0:row_max] = loss_gcn_file
    loss_gcn[i] = np.log(loss_gcn[i])
    perf_gcn[i][0:row_max] = perf_gcn_file
    perf_min[i][0:row_max] = perf_min_file
    perf_ran[i][0:row_max] = perf_ran_file
...
plt.plot(t[2][0:12],perf_gcn[2][0:12],t[2][0:12],perf_min[2][0:12],t[2][0:12], perf_ran[2][0:12])
# make these tick labels invisible
plt.xlabel(time, fontsize=fz)
plt.ylabel(performance, fontsize=fz)
plt.legend(('GNN', heuristic, 'random'),  # 'GNN-RL', 'GNN-RL-epsilon', 'min-degree'
