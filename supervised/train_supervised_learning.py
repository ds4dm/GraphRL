...
class Train_SupervisedLearning:
    ...
    def train(self, lr =0.001, epochs=0):
        ...
        torch.save(self.model.state_dict(),
                   './supervised/models/'+self.heuristic+'/SmallErgTraining/layers7_test_lr'+str(self.lr)+'/per_epochs/gcn_policy_' + self.heuristic + '_pre_' + self.train_dataset.__class__.__name__
                   + '_epochs_' + str(0) + '_cuda.pth')

        torch.save(self.model.state_dict(),
                   './supervised/models/'+self.heuristic+'/SmallErgTraining/layers7_test_lr'+str(self.lr)+'/per_steps/gcn_policy_' + self.heuristic + '_pre_' + self.train_dataset.__class__.__name__
        ...
        for epoch in range(epochs):
            ...
            if self.use_cuda:
                self.model.load_state_dict(
                    torch.load('./supervised/models/'+self.heuristic+'/SmallErgTraining/layers7_test_lr'+str(self.lr)+'/per_epochs/gcn_policy_' + self.heuristic + '_pre_' + self.train_dataset.__class__.__name__
                           + '_epochs_' + str(epoch) + '_cuda.pth'))
            else:
                self.model.load_state_dict(
                    torch.load('./supervised/models/' + self.heuristic + '/SmallErgTraining/layers7_test_lr' + str(
