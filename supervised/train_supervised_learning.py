...
class Train_SupervisedLearning:
    ...
    def train(self, lr =0.001, epochs=0):
        ...
        torch.save(self.model.state_dict(),
                   './supervised/models/'+self.heuristic+'/SmallErgTraining/layers10_test_memo_lr'+str(self.lr)+'/per_epochs/gcn_policy_' + self.heuristic + '_pre_' + self.train_dataset.__class__.__name__
                   + '_epochs_' + str(0) + '_cuda.pth')

        torch.save(self.model.state_dict(),
                   './supervised/models/'+self.heuristic+'/SmallErgTraining/layers10_test_memo_lr'+str(self.lr)+'/per_steps/gcn_policy_' + self.heuristic + '_pre_' + self.train_dataset.__class__.__name__
    ...
    def validation_loss_single_epoch(self, lr =0.001, epoch=0, val_dataset=None, dataset_type='val'):
        """
        Training function
        :param model: neural network model
        :param opt: optimizaiton function
        ...
        opt2 = optm.Adam(self.model2.parameters(), weight_decay=self.weight_d, lr=lr)
        ...
        if self.use_cuda:
            ...
            self.model.load_state_dict(
                torch.load('./supervised/models/' + self.heuristic + '/SmallErgTraining/lr' + str(
                    self.lr) + '/per_epochs/gcn_policy_' + self.heuristic + '_pre_' + self.train_dataset.__class__.__name__
                           + '_epochs_18_cuda.pth', map_location='cpu'))

        print('model_GNN_ERs loaded')
            ...
            for x in X:
                ...
                while (i < depth) and (x_model.n > 2):
                    ...
                    if not (d_min == 0 and self.prune==True):
                        ...
                        output2 = self.model2(features, m)
                        ...
                        loss_exp2GNNss = F.kl_div(output2, distribution_labels)
                        total_loss_exp2GNNss_1graph += loss_exp2GNNss.item()
                ...
                graph_no += 1
            av_loss_val += total_loss_val_1graph
            av_loss_exp2GNNss += total_loss_exp2GNNss_1graph
            av_loss_exp2rand += total_loss_exp2rand_1graph

        av_loss_val = av_loss_val/steps
        av_loss_exp2GNNss = av_loss_exp2GNNss/steps
        av_loss_exp2rand = av_loss_exp2rand/steps
        ...
        print('epochs {}'.format(epoch),
              'KLlossExp2GNN_ERsmall {}'.format(av_loss_val),
              'KLlossExp2GNN_SSsmall {}'.format(av_loss_exp2GNNss),
              'KLlossExp2Random {}'.format(av_loss_exp2rand),
              # 'MD_performance {}'.format(_val_ave_gcn)
              'GNN_real ERsmall_performance {}'.format(_val_ave_gcn)
              # self.heuristic+'_performance {}'.format(_val_ave_mind),
              # 'gcn_performance {}'.format(_val_ave_gcn)
              )
