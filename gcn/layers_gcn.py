...
class GraphConvolutionLayer_Sparse(Module):
    ...
    def __init__(self, nfeatures_in, nfeatures_out, init='xavier', bias = True):
        ...
        self.weight = nn.Parameter(nn.init.xavier_normal_(torch.Tensor(self.nfeatures_in, self.nfeatures_out).type(torch.cuda.FloatTensor if torch.cuda.is_available() else torch.FloatTensor), gain=0.02), requires_grad=True)
        ...
        if bias:
            ...
            self.bias = nn.Parameter(nn.init.xavier_normal_(torch.Tensor(1, self.nfeatures_out).type(torch.cuda.FloatTensor if torch.cuda.is_available() else torch.FloatTensor), gain=0.02), requires_grad=True)
